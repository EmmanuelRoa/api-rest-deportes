...
const Teams = () => {
  ...
  useEffect(() => {
    const fetchTeams = async () => {
      try {
        ...
        const filteredTeams = data.filter(team => team.ID_Deporte === parseInt(deporteID));
  ...
  const deporteNombre = teams.length > 0 ? teams[0].DeporteNombre : 'Teams';
  ...
  return (
    <div style={{ background: tema.background, color: tema.text, minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <NavigationButtons />
      <div style={{ flex: 1 }}>
        <h1 style={{ textAlign: 'center', padding: '24px', color: tema.text }}>{deporteNombre} Teams</h1>
